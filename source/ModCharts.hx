...
class ModCharts
{
	 ...
	 *
	 * ```haxe
	 * ModCharts.circleLoop(arrow, 90, 150);
	 * ```
	 *
	 * @param	Object 		The object to move (FlxObject)
	...
	{
		FlxTween.linearMotion(sprite, sprite.x, sprite.y, sprite.x - 100, sprite.y , duration, true, {type: FlxTween.PINGPONG, ease: FlxEase.quadInOut});
	}
	 ...
	 * @param	distancebetweennotes	The distance between each strum note(DEFAULT 150) // HAHA fuck you, default is 110. 
	...
	static public function fadeOutObject(object) {
		FlxTween.tween(object, {"alpha": 0.3}, 2);
	}
	...
	static public function fadeInObject(object) {
		FlxTween.tween(object, {"alpha": 1}, 0.2);
	}
